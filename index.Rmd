...
knitr::opts_chunk$set(
  echo = TRUE, eval = TRUE, fig.width = 10, fig.height = 3.5
)
```
...
# Dados são provenientes de normais # Lilliefors / Shapiro-Wilk
# Variâncias populacionais não são conhecidas 
# Suposição de que as variâncias populacionais são iguais # Teste F (Exige normalidade dos dados)
```
...
# Variâncias populacionais não são conhecidas # Teste F (Exige normalidade dos dados)
...
(mean(x0)-mean(y0))/((sd(x0) + sd(y0))/2)
(mean(x0)-mean(y0))/((sd.p(x0) + sd.p(y0))/2)
(mean(x0)-mean(y0))/(sd(y0))
(mean(x0)-mean(y0))/(sd.p(y0))
...
f = factor(rep(c("Treatment", "Control"), c(length(treatment), 
    length(control))), levels = c("Treatment", "Control"), 
    ordered = TRUE)
...
```{r echo=FALSE, eval=FALSE}
summary(res.aov)[[1]]$'Sum Sq'[1]/sum(summary(res.aov)[[1]]$'Sum Sq')
```
